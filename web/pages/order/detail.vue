<template>
	<view>
		...
		<view class="goods-section">
			...
			<view v-for="item,index in orderInfo.detail" :key="index">
				<view class="g-item">
					...
					<view class="left" style="font-size: 12px;color:#e93323;padding: 10rpx 30rpx;" v-if="item.apply_refund_num && item.refund_status == 1">
						{{ item.apply_refund_num }} 件退款中
					</view>
				</view>
				<view class="action-box b-t" v-if="orderInfo.pay_status == 2 || orderInfo.status == 6">
					<button class="action-btn" @click="refund(item)" v-if="orderInfo.pay_status == 2 && item.refunded_flag == 1 && orderInfo.refund_status == 1">申请退款</button>
		...
		<view class="yt-list" v-if="(orderInfo.status == 4 || orderInfo.status == 6) && goodsInfo.type == 1">
			<view class="yt-list-cell">
