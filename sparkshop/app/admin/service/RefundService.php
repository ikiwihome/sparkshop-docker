...
class RefundService
{
    ...
    public function getList(array $param): array
    {
        ...
        if (!empty($param['order_no'])) {
            $where[] = ['refund_order_no', '=', trim($param['order_no'])];
        }
     ...
     */
    public function getDetail(int $orderId, int $refundId): array
    {
     ...
     * @return array
     */
    public function checkRefundGoods(array $param): array
    {
        // 退款信息
     ...
     * @return array
     */
    public function checkRefundMoney(array $param): array
    {
        // 退款信息
        $refundModel = new OrderRefund();
        $refundInfo = $refundModel->findOne([
     ...
     * @return array
     */
    private function denyRefund($param, $refundInfo, $refundModel): array
    {
     ...
     * @return array
     */
    private function agreeRefund($param, $refundInfo, $refundModel): array
    {
        // 查询对应的订单信息
        $orderModel = new Order();
        ...
        try {
            ...
            if ($refundInfo['refund_way'] == 2) {
                ...
                if ($orderInfo['experience'] > 0) {
                    ...
                    $orderParam['experience'] = 0; // 把赠送的经验值全部清空
                }
    ...
    private function changeStatusAndUpdateCoupon($orderInfo)
    {
        ...
        // 如果存在消耗的优惠券，则退回优惠券 TODO V1.1.3改造事件机制
        if ($couponInstalled) {
     ...
     * @return array
     */
    private function splitOrder($orderInfo, $refundInfo): array
    {
        try {
            // 原订单所有的子订单数据
            $orderDetailModel = new OrderDetail();
     ...
     * @throws DbException
     */
    private function rebuildOrder($orderInfo, $detailList, $refundInfo): array
    {
        ...
        $orderAddressInfo = $orderAddressModel->findOne(['order_id' => $orderInfo->id])['data'];
        // 重新绑定收货地址
