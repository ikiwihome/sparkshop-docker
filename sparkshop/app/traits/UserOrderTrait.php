...
trait UserOrderTrait
{
     ...
     * @return array
     */
    private function dealGoPay(array $param, $orderInfo, Order $orderModel, int $userId): array
    {
     ...
     * @return array
     */
    private function dealRefundOrder(array $param, int $userId, string $userName): array
    {
        ...
        if (!empty($orderInfo['received_time'])) {
            ...
            if ((time() - strtotime($orderInfo['received_time'])) > $validateDay * 86400) {
                $orderModel->updateById(['refund_status' => 3], $param['order_id']); // 设置为不可退款
     ...
     * @return array
     */
    private function refundMainOrder(array $param, int $userId, string $userName, $orderInfo): array
    {
        $orderModel = new Order();
     ...
     * @return array
     */
    private function refundSubOrder(array $param, int $userId, string $userName, $orderDetailList): array
    {
        $orderModel = new Order();
        $orderRefundModel = new OrderRefund();
