...
class GoodsService
{
     ...
     * @return array
     */
    public function addGoods(array $param, string $mode): array
    {
     ...
     * @return array
     */
    public function editGoods(array $param, string $mode): array
    {
        $res = $this->checkParam($param);
        if ($res['code'] != 0) {
            return $res;
     ...
     * @return array
     */
    private function goodsTemplate(array $param, string $mode): array
    {
    ...
    private function checkParam(array $param): array
    {
        ...
        if ($param['spec'] == 1) {
            ...
            if (!isset($param['id']) && ($param['stock'] == '' || $param['stock'] < 0)) {
            ...
            foreach ($param['final'] as $vo) {
                ...
                if (!isset($param['id']) && ($vo['stock'] == '' || $vo['stock'] < 0)) {
                    $errorMsg = '规格库存要大于等于0';
                    break;
                }
     ...
     * @return array
     */
    private function dealBase(array $param, string $mode): array
    {
        $goodsModel = new Goods();
        if ($mode == 'edit') {
     ...
     * @return array
     */
    private function dealRule(array $param, string $mode): array
    {
        $goodsStock = 0;
        $priceMap = [];
        $originalPriceMap = [];
        ...
        if ($param['spec'] == 2) {
            ...
            if ($mode == 'edit') {
                $has = $goodsRuleModel->findOne(['goods_id' => $param['id']], 'id')['data'];
                if (empty($has)) {
     ...
     * @return array
     */
    private function dealContent(array $param, string $mode): array
    {
        $contentMap = [
            'goods_id' => $param['goods_id'],
     ...
     * @return array
     */
    private function dealExpress(array $param): array
    {
        $freightMap = [
