...
class CouponReceiveService
{
     ...
     * 获取我的可用的优惠券
     * @param array $param
     * @param int $type 1:小程序 2:pc
     * @return array
     */
    public function getMyCoupon(array $param, int $type = 1): array
    {
        $goodsLimit = [];
        ...
        $couponReceiveLogModel->updateByWhere([
            'status' => 3,
            'update_time' => now()
        ], ['end_time', '<', now()]);
        ...
        $param['amount'] = floatval(preg_replace('/[^\d.]/', '', $param['amount'])); // 反格式化金额
     ...
     * @throws DbException
     */
    public function getCouponReceiveLog(array $param): array
    {
        $limit = $param['limit'];
        $status = $param['status'];
     ...
     * @return array
     */
    public function userReceive(array $param, int $type = 1): array
    {
        Db::startTrans();
        try {
