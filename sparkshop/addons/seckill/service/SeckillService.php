...
class SeckillService
{
    ...
    public function getList(array $param): array
    {
        ...
        // 一键关闭过期的
        $seckillTimeModel->updateByWhere([
            'status' => 3, // 已结束
            'update_time' => now()
        ], [
     ...
     * @return array
     */
    public function addSeckillGoods(array $param): array
    {
        ...
        try {
            ...
            // 获取活动商品主图
            $picList = (new Goods())->field('slider_image')->where('id', $param['goods_id'])->find();

            $activity = [
                'goods_id' => $param['goods_id'],
                'goods_rule' => $param['goods_rule'],
                'pic' => json_decode($picList['slider_image'], true)[0],
                'name' => $param['name'],
                'desc' => $param['desc'],
     ...
     * @return array
     */
    public function editSeckillGoods(array $param): array
    {
        $checkParam = $this->checkParam($param);
        if ($checkParam['code'] != 0) {
            return $checkParam;
     ...
     */
    public function delSeckill(int $id): array
    {
        $activityModel = new SeckillActivity();
     ...
     * @throws DbException
     */
    public function getSeckillList(array $param): array
    {
        $limit = $param['limit'];
     ...
     * @return array
     */
    public function seckillDetail(int $id): array
    {
        // 检测秒杀参数的合法性
     ...
     * @return array
     */
    protected function checkParam(array $param): array
    {
        try {
     ...
     * @throws ModelNotFoundException
     */
    public function buildEditParam(int $activityId): array
    {
        $seckillTimeModel = new SeckillTime();
