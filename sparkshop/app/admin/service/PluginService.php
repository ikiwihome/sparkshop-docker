...
class PluginService
{
     ...
     * @return array
     */
    public function getAppList(array $param): array
    {
        $where = [];
     ...
     * @return array
     */
    public function createPlugin(array $param): array
    {
        try {
        ...
        $content = str_replace('<<icon>>', $param['icon'], $content);
     ...
     * @return array
     */
    public function uninstallPlugin(int $id): array
    {
        $pluginsModel = new Plugins();
        ...
        // 卸载菜单
        return (new AdminNode())->updateByWhere([
            'status' => 2, // 禁用
            'update_time' => now()
     ...
     * @return array
     */
    public function installPlugin(int $id): array
    {
        $pluginsModel = new Plugins();
        $pluginInfo = $pluginsModel->findOne(['id' => $id])['data'];
        if (empty($pluginInfo)) {
        ...
        return $nodeModel->updateByWhere([
            'status' => 1, // 启用
            'update_time' => now()
        ], ['type' => $pluginInfo['name']]);
    }
     ...
     * @return array
     */
    public function deletePlugin(int $id): array
    {
        $pluginsModel = new Plugins();
        $pluginInfo = $pluginsModel->findOne(['id' => $id])['data'];
        if (empty($pluginInfo)) {
     ...
     * @return array
     */
    public function doPack(string $name): array
    {
        ...
        try {
            SparkTools::dirZip($name . '.zip', $dir);
        } catch (\Exception $e) {
     ...
     * @return array
     */
    public function makeUpdateZip(array $param): array
    {
        ...
        try {
            ...
            SparkTools::dirZip($fileName, $updateDir);
     ...
     * @return array
     */
    public function getDirInfo(string $name, string $dir): array
    {
        if (empty($dir)) {
            $dir = App::getRootPath() . 'addons' . DS . $name;
        }
     ...
     * @return array
     */
    public function updateOrInstall(array $param): array
    {
        $flag = unzip($param['package_url'], App::getRootPath() . 'addons' . DS . $param['name']);
        if (!$flag) {
     ...
     * @return array
     */
    protected function dealInstall(array $param): array
    {
        $pluginsModel = new Plugins();
     ...
     * @return array|mixed
     */
    protected function dealUpdate(array $param)
    {
