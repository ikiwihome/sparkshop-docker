...
class OrderService
{
     ...
     * @return array
     */
    public function seckillGoodsInfo(array $param): array
    {
     ...
     * @return array
     */
    public function trail(array $param, int $type = 1): array
    {
        // 商品信息
     ...
     * @return array
     */
    public function checkSeckill(int $seckillId): array
    {
        // 获取秒杀信息
    ...
    public function createOrder(array $param, int $type = 1): array
    {
        ...
        if (empty($userInfo)) {
            return dataReturn(-13, '请先登陆');
        }

        // 检测下单条件
        $checkRes = $this->preCheck($redis, $orderData, $userInfo);
        if ($checkRes['code'] != 0) {
            return $checkRes;
        }
        ...
        try {
            ...
            // 执行创建订单
            $orderCreateRes = $this->dealCreateOrder($param, $orderData, $seckillActivityInfo, $type);
            ...
            $redis->incrby($key, $orderData['num']); // 还原redis库存
            Db::rollback();
            return dataReturn(-12, '系统错误' . $e->getMessage() . '|' . $e->getFile() . '|' . $e->getLine());
        }
    }
     ...
     * @throws DbException
     */
    protected function preCheck(\Redis $redis, array $orderData, array $userInfo): array
    {
        // 用户过往买过的该秒杀商品
     ...
     * @return array
     */
    private function dealCreateOrder(array $param, array $orderData, $seckillActivityInfo, int $type): array
    {
        try {
            // 开始下单
            if ($type == 1) {
            ...
            // 试算
            $calcParam = $this->trail($param, $type)['data'];
            ...
            return dataReturn(-11, '下单异常' . $e->getMessage() . '>>>' . $e->getFile() . '>>>' . $e->getLine());
        }
     ...
     * @return void
     */
    private function writeOverdueData(int $orderId, int $num)
    {
        $overdueModel = new OrderOverdue();
     ...
     * @return array
     */
    private function completeOrder($orderInfo): array
    {
        try {
     ...
     */
    public function dealStockAndSales($orderInfo): bool
    {
     ...
     */
    public function refundStockAndSales($orderInfo): bool
    {
        $seckillActivityGoodsModel = new SeckillActivityGoods();
