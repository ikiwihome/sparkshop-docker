...
function trial() {
    $.post('/index/order/trial', orderParam, function (res) {
        if (res.code == 0) {
            ...
            $('#balance').text(res.data.user_balance)
...
function checkCoupon() {
    layui.use(['layer', 'laytpl'], function () {
        ...
        $.post('/addons/coupon/index.index/getMyValid', {amount: totalPrice, goods: JSON.stringify(orderParam.goods)},
            function (res) {
...
// 优惠券选定
function selectedCoupon(obj) {
    layer.close(couponLayer)
    let coupon = JSON.parse($(obj).attr('data-coupon'))
    ...
    _html += '<div class="cell-left">' + typeTxt + '</div>';
