...
function fetch($tpl = '', $data = [])
{
    ...
    if (empty($tpl)) {
        if (strstr($pathInfo[3], '.html')) {
            $tpl = substr($pathInfo[3], -5);
        } else {
            $tpl = $pathInfo[3];
        }
    }
...
function getUserInfoInPlugin() {
    ...
    $sessionConfig = include App()->getAppPath() . 'index' . DS . 'config' . DS . 'session.php';
    $sessionConfig['path'] = App()->getRuntimePath() . 'index' . DS . 'session' . DS;
...
function hasInstalled($flag) {
    $info = (new Plugins())->findOne(['name' => $flag, 'status' => 2])['data'];
    return !empty($info);
}
