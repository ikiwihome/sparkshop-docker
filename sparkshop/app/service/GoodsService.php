...
class GoodsService
{
     ...
     * @throws ModelNotFoundException
     */
    public function getMobileGoodsDetail(int $goodsId): array
    {
        $goodsModel = new Goods();
        $goodsDetail = $goodsModel->findOne([
            'id' => $goodsId,
            'is_show' => 1,
            'is_del' => 2
        ], 'name,spec,price,sales,slider_image,stock,unit,views,original_price,type')['data'];
        if (empty($goodsDetail)) {
            return dataReturn(-10, '商品不存在');
        }
        $goodsDetail['slider_image'] = json_decode($goodsDetail['slider_image'], true);
     ...
     * @return array
     */
    public function getGoodsRuleDetail(string $sku, int $goodsId): array
    {
        $goodsRuleExtendModel = new GoodsRuleExtend();
     ...
     * @throws ModelNotFoundException
     */
    public function getComments(array $param): array
    {
        $limit = $param['limit'];
        $type = $param['type'];
     ...
     * @return array
     */
    public function getGoodsByCateId(array $param): array
    {
        $limit = $param['limit'];
     ...
     * @return array
     */
    protected function formatRule($goodsRuleExtend): array
    {
        $res = [];
     ...
     * @return array[]
     */
    protected function powerSet($item): array
    {
        $ps = [[]];
