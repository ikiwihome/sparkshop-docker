	...
	<view class="container">
		...
		<view class="introduce-section">
			...
			<view class="price-box">
				...
				<div style="margin-left:auto;" @click="share" v-if="nowUserId > 0 && canShare">
					<uni-icons type="redo-filled" size="23" color="#e93323"></uni-icons>
				</div>
			</view>
			<view class="bot-row">
		...
		<view class="page-bottom">
			...
			<view class="action-btn-group" v-if="goodsInfo.type != 2">
			...
			<view class="action-btn" v-else>
				<button type="primary" class="action-btn no-border" @click="buy" style="width: 200px;background: #e93323;">立即购买</button>
			</view>
		</view>
		...
		<uni-popup ref="poster" type="bottom">
			<view class="poster-box">
				<!--<view class="poster-menu-item">
					<uni-icons type="weixin" size="40" color="#5eae72"></uni-icons><br/>
					分享给好友
					<button id="shareBtn" open-type="share" style="width: 40%;"></button>
				</view>-->
				<view class="poster-menu-item" @click="makePoster">
					<uni-icons type="image-filled" size="40" color="#5391f1"></uni-icons><br/>
					生成海报
				</view>
			</view>
		</uni-popup>
		...
		<view class="poster-pop" v-if="showPoster">
		...
		<view class="mask" v-if="showPoster"></view>

		<canvas class="canvas" canvas-id="myCanvas" v-show="canvasStatus"></canvas>
		<canvas class="canvas" canvas-id="qrcode" v-show="qrStatus"/>
	</view>
</template>
...
<script>
	...
	// #ifdef H5
	import uQRCode from '@/components/qrcode/qrcode.js' // 引入uqrcode.js
	// #endif
	import {
		UID
	} from '@/config/cache';
	...
	export default{
		...
		async onLoad(options) {
			let id = 0
			// 通过分享进入 scene值是id=1&spId=1 id是商品id spId是推广人id
			// #ifdef MP || APP-PLUS
			let scene = options.scene
			if (scene) {
				scene = decodeURIComponent(scene)
				let sceneMap = scene.split('&')
				id = this.goodsId = sceneMap[0].split('=')[1]
				this.spId = sceneMap[1].split('=')[1]
			} else {
				id = this.goodsId = options.id
			}
			// #endif
			
			// #ifdef H5
			id = this.goodsId = options.id
			this.spId = options.spId ? options.spId : 0
			// #endif
	
			// 当前用户
			this.nowUserId = Cache.get(UID) ? Cache.get(UID) : 0
			// 存储推广用户
			if (this.nowUserId == 0 && this.canShare) {
				uni.setStorageSync('spId', this.spId)
			} else {
		...
		methods: {
			// 获取基础配置
			async getConfig() {
				let res = await this.$api.order.getConfig.get({goods_id: this.goodsId})
				this.couponOpen = res.data.coupon
				if (this.couponOpen == 1) {
					this.getCouponList()
				}
				this.canShare = res.data.agent == 1
			},
			// 商品详情
			...
			async makePoster() {
				...
				uni.downloadFile({
					url: this.imgList[0],
					success: async (res) => {
						let qrcode = await this.$api.agent.getQrCode.post({goods_id: this.goodsId, spId: this.nowUserId})
						uni.downloadFile({
							url: qrcode.data,
							success: (res2) => {
								let imgs = [
									'/static/poster-bg.png',
									res.tempFilePath, // 产品图片
									res2.tempFilePath // 太阳码
								];
								this.toMakePoster(imgs)
							}
						});
					}
				});
				// #endif
				// #ifdef H5
				let goodsImg = await this.$api.common.path2Base64.post({path: this.imgList[0]})
				let qrcode = ''
				let imgs = [
					'/static/poster-bg.png',
					goodsImg.data,
				...
				uQRCode.make({
					...
					text: window.location.href + '&spId=' + this.nowUserId,
					size: 220,
					margin: 0,
			...
			toMakePoster(imgs) {
				utils.poster2canvas(imgs, this.goodsInfo.name, this.goodsInfo.price, this.goodsInfo.original_price, (res) => {
					uni.hideLoading()
							...
							uni.authorize({
								...
								success() {
									uni.saveImageToPhotosAlbum({
										filePath: that.posterSrc,
										success: function(res) {
											that.showPoster = false
											that.$tool.msg('保存成功', 1500, false, 'success')
										},
										fail: function(res) {
											that.$tool.msg('保存失败', 1500, false, 'error')
										},
									});
								},
							});
