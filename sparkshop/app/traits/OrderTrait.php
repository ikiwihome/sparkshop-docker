...
trait OrderTrait
{
     ...
     * @return array
     */
    private function dealTrial(array $param, int $userId): array
    {
        try {
     ...
     * @return mixed
     */
    private function dealCreateOrder(array $param, int $userId)
    {
        try {
            ...
            if ($goodsType == 1 && empty($param['address_id'])) {
                return dataReturn(-6, '请选择收货地址');
            }
     ...
     */
    private function dealStockAndSales($orderInfo): bool
    {
     ...
     * @throws ModelNotFoundException
     */
    public function giveBackSaleAndStore($refundInfo, $orderInfo): array
    {
        // 普通订单
        if ($orderInfo['type'] == 1) {
     ...
     * @param array $goods2Price
     * @param int $userId
     * @return array
     */
    private function calcCoupon(array $param, int $totalPrice, array $goods2Price, int $userId): array
    {
     ...
     * @return void
     */
    private function writeOverdueData(int $orderId, int $orderType, $postGoodsInfo)
    {
    ...
    /**
     * 计算优惠券均摊
     * @param array $calcParam
     * @param array $orderGoodsList
     * @param array $goods2Num
     * @return array
     */
    protected function calcDivideCoupon(array $calcParam, array $orderGoodsList, array $goods2Num): array
    {
        $goods2Amount = [];
        // 通用券
     ...
     * @return array
     */
    protected function calcDivideUserVip(array $calcParam, array $orderGoodsList, array $goods2Num): array
    {
        $vip2Amount = [];
        $hasDivideAmount = 0;
        $i = 0;
