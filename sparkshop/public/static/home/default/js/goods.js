    ...
    $('.item-items li').click(function () {
        ...
        if (ruleJson[nextIndex]) {
            ...
            for (var i = 0; i < item.length; i++) {
                let itemKey = selectedMap.join("※") + "※" + item[i];
...
function getValidCoupon() {
    $.getJSON('/addons/coupon/index.index/getCouponList', {goods_id: goods_id}, function (res) {
        if (res.code == 0 && res.data.length > 0) {
            ...
            $.each(res.data, function (k, v) {
               ...
               _html += '<div class="cell">' +
                   '        <div class="cell-left">' + type_txt + '</div>' +
...
function receive(couponId) {
    ...
    $.post('/addons/coupon/index.index/receive', {id: couponId}, function (res) {
        if (res.code == 0) {
            notify.success(res.msg);
            getValidCoupon();
        } else if (res.code == 403) {
    ...
    layui.use(['laytpl'], function () {
        ...
        $.getJSON('/index/goods/getComments', {
            ...
            if (res.code == 0) {
                ...
                if (res.data.list.last_page == page || res.data.list.total == 0) {
                    $('#moreComment').hide();
                } else {
                    $('#moreComment').show();
                }
